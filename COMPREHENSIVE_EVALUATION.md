...
**現状:** 指定repo内のCPP05〜CPP09は、current EvalHub HTML criteria、全source、最終fresh evidenceの確認範囲でblockerなし・code-ready。実際の提出可否は、Intraが参照する公式repo/branchと監査済みsourceの同一性確認を条件とする。

## 1. 評価基準とprovenance

優先順位は次のとおりである。

1. 2026-09-02に実ブラウザで確認したcurrent 42 EvalHub HTML criteria
...
| [CPP05](https://www.42evalhub.com/common/cpp05) | Bureaucrat、Form、AForm、3 concrete forms、`executeForm`。Internはcreator function/member-function pointer tableでdispatchしif/else-if chain不可、未知名は明示エラー。                                           |
| [CPP06](https://www.42evalhub.com/common/cpp06) | ScalarConverterはprivate constructor/static cast、Serializerはprivate/static reinterpret_cast、identifyはpointer NULL/reference try-catch dynamic_cast、`typeinfo`禁止。                             |
| [CPP07](https://www.42evalhub.com/common/cpp07) | whateverのsimple+complex、iterのevaluator harness、Arrayの`new[]`、empty/sized/default initialization/read-write/const/bounds/simple+complex/deep copy。                                           |
| [CPP08](https://www.42evalhub.com/common/cpp08) | easyfindはSTL algorithm、SpanはSTL algorithm+range add、MutantStack iteratorとsubject以上のtest。                                                                                                    |
| [CPP09](https://www.42evalhub.com/common/cpp09) | btcはmap/closest lower/empty+bad lines継続/invalid date/`<0`/`>1000`/`input.csv`。RPNは別containerとadvanced `42`/`42`/`15`。PmergeMeは未使用の2 containers、両方Ford–Johnson、5〜10手動+3000 random、timing差説明。 |
...
PASSは、指定repoのcurrent EvalHub HTML criteriaと取得可能な履歴PDF、現行実装、最終fresh evidenceの確認範囲でblockerがないことを表す。Intraが参照する公式repo/branchとの同一性や、CDNが404の最新subject PDF現物まで確認済みという意味ではない。
...
- `AForm::execute`は署名・gradeを共通検査し、concrete formはactionに限定する。
Internはnameとstatic creator function pointerのtableをloopして選ぶ。
- 2026-08-24の既存修正（Intern未知名出力）は維持され、未知名を明示エラーとして一度だけ報告し、既知formへ誤dispatchしない。
...
- 2026-08-24の既存修正（Array mainのcomplex/default proof）は維持され、複雑型とdefault initializationをmain/harnessで実証する。
...
## 4. 最終fresh evidence（2026-09-02）


| 環境 / check                        | 最終結果                                                                                                                                                  |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| 監査対象source baseline               | `e04faff6cdb55e15fa7be1f287fc512235a9d742`。監査開始時はGitHub/local `main`が一致し、worktree clean。                                                              |
| macOS 26.6.2 / Apple clang 21     | `./scripts/verify_cpp05_09.sh`: 176 PASS / 0 FAIL / 1 SKIP（Valgrind unavailableのみ）。                                                                   |
| macOS ASan + UBSan                | 全16 binary: 16/16 PASS。Apple ASanは`detect_leaks=0`で実行。                                                                                                |
| current EvalHub `input.csv`       | 22行（header+21 data）をbtcがexit 0で処理、21 output lines。exact 2011-09-14=6.19、closest-lower 2010-11-03→2010-11-02=0.21、2020-06-26→2020-06-25=9276.58を手計算照合。 |
...
| 静的/ソース検査                          | 16 Exerciseの全source、禁止構文、26 header単体、Makefile/compile flagsをcurrent EvalHub項目ごとに確認しPASS。                                                              |
...
この測定は2026-09-02 fresh evidenceと混同しない。監査用一時copyで値比較をinstrumentし、n=1〜10の全4,037,913 permutationを列挙した。提出実装にはinstrumentを残していない。
...
- CPP05〜09のcurrent EvalHub subject.pdf CDNは全て404で、CPP09の`cpp_09.tgz`も404だった。current HTML以外の最新配布物はこの評価で確認できていない。CPP09の`input.csv`のみHTTP 200で取得・実行した。
- 履歴PDFは実測SHA-256を記録した旧版であり、current EvalHub PDF現物の代替ではない。
- 2026-09-02のfresh macOS値は176 PASS / 0 FAIL / 1 SKIPである。193 PASS / 0 FAIL / 0 SKIPは2026-08-24 Ubuntu/Valgrindの履歴証拠である。
...
指定repo内のCPP05〜CPP09はcode-readyであり、2026-09-02のcurrent EvalHub HTML criteria、全source、最終fresh evidenceの確認範囲でblockerはない。コード修正は不要だった。実際に提出する公式repo/branchが監査済みsourceと一致すれば提出可能・defense-readyと判定する。これは公式repoとの一致や、CDNが404のcurrent EvalHub subject PDF現物を確認済みとする主張ではない。
